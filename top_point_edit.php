            ...
            if (isset($_GET['SQL'])) {
                ...
                if (mb_strlen($errormsg) == 0) {
                    ...
                    try {
                        ...
                        $sql = 'UPDATE POINTPOSITION_MASTER SET POINTFIELD_NAME=?, POINTFIELD_QRCODE=?, MASTER_CODE=?, UPLOAD_DATE=? WHERE POINTFIELD_NO=?';
    ...
    if (isset($_GET['SQL'])) {
        ...
        if (mb_strlen($errormsg) == 0) {
            try {
                ...
                $sql = 'DELETE FROM POINTPOSITION_MASTER WHERE POINTFIELD_NO=?';
                $stmt = $dbh->prepare($sql);
                $data[] = $POINTFIELD_NO;
                    ...
                    <div class="col-12 col-xl-12 mb-4 mb-lg-0">
                        <div class="card">
                            <div class="card-body">
                                ...
                                <form action="top_point_edit.php">
                                    <div class="input-group mb-3">
                                        <span class="input-group-text" id="inputGroup-sizing-default">ポイント配布場所番号</span>
                                        <input name="POINTFIELD_NO" type="text" class="form-control" aria-label="Sizing example input" aria-describedby="inputGroup-sizing-default" placeholder="ポイント配布場所番号" value=<?= $POINTFIELD_NO ?> readonly>
                                    </div>
                                    <div class="input-group mb-3">
                                        <span class="input-group-text" id="inputGroup-sizing-default">ポイント配布場所の名前</span>
                                        <input name="POINTFIELD_NAME" type="text" class="form-control" aria-label="Sizing example input" aria-describedby="inputGroup-sizing-default" placeholder="配布場所の名前（100文字まで）" value=<?= $POINTFIELD_NAME ?>>
                                    </div>
                                    <div class="input-group mb-3">
                                        <span class="input-group-text" id="inputGroup-sizing-default">ポイントのQRコード</span>
                                        <input name="POINTFIELD_QRCODE" type="text" class="form-control" aria-label="Sizing example input" aria-describedby="inputGroup-sizing-default" placeholder="QRコード（30文字まで）" value=<?= $POINTFIELD_QRCODE ?>>
                                    </div>
                                ...
                                <form action="top_point_edit.php">
                                    <input type="hidden" name="POINTFIELD_NO" value=<?= $POINTFIELD_NO ?> />
