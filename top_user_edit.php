                    ...
                    if (isset($_GET['SQL'])) {
                        ...
                        if ($SQL == "UPDATE") {
                            if (mb_strlen($errormsg) == 0) {
                                try {
                                    $STUDENT_NO = htmlspecialchars($STUDENT_NO, ENT_QUOTES, 'UTF-8');
                                    ...
                                    //SQL実行
                                    $sql = 'UPDATE STUDENT_MASTER SET DEPERTMENT_CODE=?, STUDENT_CODE=?, NAME=?, MAIL=?, MASTER_CODE=?, UPLOAD_DATE=? WHERE STUDENT_NO=?';
        ...
        if ($SQL == "DELETE") {
            if (mb_strlen($errormsg) == 0) {
                try {
                    ...
                    //SQL実行
                    $sql = 'DELETE FROM STUDENT_MASTER WHERE STUDENT_NO=?';
                    $stmt = $dbh->prepare($sql);
                    $data[] = $STUDENT_NO;
                    ...
                    <div class="col-12 col-xl-12 mb-4 mb-lg-0">
                        <div class="card">
                            <div class="card-body">
                                ...
                                <form action="top_user_edit.php">
                                    <div class="input-group mb-3">
                                        <span class="input-group-text" id="inputGroup-sizing-default">社員コード</span>
                                        <input name="DEPERTMENT_CODE" type="text" class="form-control" aria-label="Sizing example input" aria-describedby="inputGroup-sizing-default" placeholder="GK1234567890（12文字まで）" value=<?= $DEPERTMENT_CODE ?>>
                                    </div>
                                    <div class=" input-group mb-3">
                                        <span class="input-group-text" id="inputGroup-sizing-default">社員名</span>
                                        <input name="STUDENT_CODE" type="text" class="form-control" aria-label="Sizing example input" aria-describedby="inputGroup-sizing-default" placeholder="GS1234567890（12文字まで）" value=<?= $STUDENT_CODE ?>>
                                    </div>
                                    <div class="input-group mb-3">
                                        <span class="input-group-text" id="inputGroup-sizing-default">名前</span>
                                        <input name="NAME" type="text" class="form-control" aria-label="Sizing example input" aria-describedby="inputGroup-sizing-default" placeholder="あいうえお（50文字まで）" value=<?= $NAME ?>>
                                    </div>
                                    <div class="input-group mb-3">
                                        <span class="input-group-text" id="inputGroup-sizing-default">メールアドレス</span>
                                        <input name="MAIL" type="text" class="form-control" aria-label="Sizing example input" aria-describedby="inputGroup-sizing-default" placeholder="abc@dmain.com（100文字まで）" value=<?= $MAIL ?>>
                                    </div>
                                ...
                                <form action="top_user_edit.php">
                                    ...
                                    <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                                        <input type="submit" class="btn btn-danger btn-lg" value="削除" />
                                    </div>
                                </form>
                            </div>
                        </div>
